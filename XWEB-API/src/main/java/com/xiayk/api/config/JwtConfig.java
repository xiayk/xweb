...
public class JwtConfig {
    ...
    private final String TOKEN_KEY = "XWEB_TOKEN";
     ...
     * @return 返回 jwt token
     */
    public String createTokenAccount(String username) {
        //JWT 随机ID,做为验证的key
    ...
    public boolean verifyToken(String token) {
        try {
            ...
            verifier = JWT.require(algorithm)
                    ...
                    .withJWTId(getJwtIdByToken(token))
                    .withSubject(getUsernameByToken(token))
                    //JWT 正确的配置续期姿势
    ...
    /**
     * 根据Token获取username(注意坑点 : 就算token不正确，也有可能解密出wxOpenId,同下)
     */
    ...
    public String getTokenType(HttpServletRequest request){
        String auth = getUsernameByToken(getTokenType(request));
        if (auth.length() >= 20){
            return null;
        }
        return "";
